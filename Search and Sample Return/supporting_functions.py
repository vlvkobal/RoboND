...
def update_rover(Rover, data):
      ...
      if Rover.start_time == None:
            ...
            Rover.init_pos = [convert_to_float(pos.strip()) for pos in data["position"].split(';')]
...
def create_output_images(Rover):
      ...
      cv2.putText(map_add,"  Collected: "+str(Rover.samples_collected), (0, 85), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      if Rover.mode == 'parked':
            cv2.putText(map_add,"  Finished", (0, 160), 
                        cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
            cv2.putText(map_add,"  in "+str(np.round(Rover.finish_time, 1))+' s', (0, 175), 
                        cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
