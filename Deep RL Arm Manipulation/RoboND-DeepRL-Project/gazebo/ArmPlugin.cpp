...
#define ALPHA 0.3f
#define REWARD_WIN  0.5f
#define REWARD_LOSS -lastGoalDistance
//#define REWARD_LOSS -1.0f -100.0f * sqrt(lastGoalDistance)
#define REWARD_FRAMES_EXCEEDED REWARD_LOSS
#define REWARD_GROUND_COLLISION REWARD_LOSS
#define REWARD_ARM_COLLISION REWARD_LOSS
#define REWARD_GRIPPER_COLLISION REWARD_WIN
//#define REWARD_GRIPPER_COLLISION REWARD_WIN + maxEpisodeLength - episodeFrames - 1000 * avgGoalDelta
#define REWARD_INTERIM avgGoalDelta
//#define REWARD_INTERIM 80 * avgGoalDelta

// Define Object Names
#define WORLD_NAME "arm_world"
#define PROP_NAME  "tube"
#define GRIP_NAME  "gripper_middle"
#define GRIP_BASE_NAME  "gripperbase"
...
{
	//if(DEBUG){printf("ArmPlugin - Camera message\n");}
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		{
			std::cout << "delta = " << avgGoalDelta << "\n";
			rewardHistory = REWARD_GRIPPER_COLLISION;
		...
		if (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0)
		{
			if(DEBUG){printf("ARM CONTACT, EOE\n");}

			std::cout << "delta = " << avgGoalDelta << "\n";
			rewardHistory = REWARD_ARM_COLLISION;
			newReward     = true;
			endEpisode    = true;
...
{
	//if(DEBUG){printf("ArmPlugin - updateAgent\n");}
...
{
	//if(DEBUG){printf("ArmPlugin - updateJoints\n");}
...
{
	//if(DEBUG){printf("ArmPlugin - OnUpdate\n");}
	...
	if( maxEpisodeLength > 0 && episodeFrames > maxEpisodeLength )
	{
		...
		rewardHistory = REWARD_FRAMES_EXCEEDED;
		newReward     = true;
		endEpisode    = true;
	}
	...
	if( hadNewState && !newReward )
	{
		...
		physics::LinkPtr gripperbase  = model->GetLink(GRIP_BASE_NAME);
		...
		const math::Box& gripbaseBBox = gripperbase->GetBoundingBox();
		...
		*/
		...
		if(gripBBox.min.z <= groundContact)
		{
			...
			std::cout << "delta = " << avgGoalDelta << "\n";
			rewardHistory = REWARD_GROUND_COLLISION;
			newReward     = true;
			endEpisode    = true;
		}
		...
		*/ 
		...
		if(gripBBox.min.z > groundContact)
		{
			const float distGoal = BoxDistance(gripbaseBBox, propBBox); // compute the reward from distance to the goal
			...
			if( episodeFrames > 1 )
			{
				...
				//std::cout << "INTERIM DISTANCE = " << distGoal << "\n";
				newReward     = true;	
			}
